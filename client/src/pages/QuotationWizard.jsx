...
function EditableAttributeTags({ values = [], options, onChange, label }) {
    ...
    return (
        ...
        <div ref={ref} style={{ position: 'relative', display: 'inline-block', zIndex: open ? 9999 : 'auto' }}>
            ...
            {/* Ã¢€—Ã¢€— Dropdown — plain conditional render, NO AnimatePresence Ã¢€—Ã¢€— */}
...
function RecCell({ label, rec, onChange, customLabels = {}, onRenameLabel, itemId, hiddenCols = {}, setHiddenCols, products = [] }) {
        ...
        <td style={{ verticalAlign: 'top', padding: '0 4px', minWidth: 140 }}>
            ...
            <div style={{ background: 'var(--color-base)', border: `1px solid ${color}33`, borderTop: `3px solid ${color}`, borderRadius: 8, padding: '8px 6px' }}>
                ...
                <div style={{ marginBottom: 5 }}>
                    ...
                    <div style={{ fontSize: 9, color: '#374151', marginBottom: 2, letterSpacing: 0.3, display: 'flex', alignItems: 'center', gap: 4 }}>

                        <span style={{ textDecoration: hiddenCols['Listing Price'] ? 'line-through' : 'none', opacity: hiddenCols['Listing Price'] ? 0.5 : 1 }}>

                            {getCustomLabel(customLabels, `rec_${itemId}_${label}_price`, 'Listing Price')}

                        </span>

                        <button type="button" onClick={() => setHiddenCols(prev => ({ ...prev, 'Listing Price': !prev['Listing Price'] }))}

                            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', padding: 0 }}>

                            {hiddenCols['Listing Price'] ? <EyeOff size={10} /> : <Eye size={10} />}
                ...
                {editableFields.map(f => (
                    ...
                    <div key={f.key} style={{ marginBottom: 5 }}>
                        ...
                        <div style={{ fontSize: 9, color: '#374151', marginBottom: 2, letterSpacing: 0.3, display: 'flex', alignItems: 'center', gap: 4 }}>
                            ...
                            <span style={{ textDecoration: hiddenCols[f.label] ? 'line-through' : 'none', opacity: hiddenCols[f.label] ? 0.5 : 1 }}>

                                {getCustomLabel(customLabels, `rec_${itemId}_${label}_${f.key}`, f.label)}

                            </span>

                            <button type="button" onClick={() => setHiddenCols(prev => ({ ...prev, [f.label]: !prev[f.label] }))}

                                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', padding: 0 }}>

                                {hiddenCols[f.label] ? <EyeOff size={10} /> : <Eye size={10} />}
                        ...
                        {f.readOnly ? (
                            ...
                            <div style={{ padding: '3px 6px', fontSize: 11, fontWeight: 600, color: f.key === 'amount' ? color : 'var(--color-text-secondary)', background: 'var(--color-elevated)', borderRadius: 4, opacity: hiddenCols[f.label] ? 0.6 : 1 }}>
                ...
                <div style={{ marginBottom: 5 }}>
                    ...
                    <div style={{ fontSize: 9, color: 'var(--color-text-muted)', marginBottom: 2, letterSpacing: 0.3, display: 'flex', alignItems: 'center', gap: 4 }}>

                        <span style={{ textDecoration: hiddenCols['Unit'] ? 'line-through' : 'none', opacity: hiddenCols['Unit'] ? 0.5 : 1 }}>Unit</span>

                        <button type="button" onClick={() => setHiddenCols(prev => ({ ...prev, 'Unit': !prev['Unit'] }))}

                            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', padding: 0 }}>

                            {hiddenCols['Unit'] ? <EyeOff size={10} /> : <Eye size={10} />}

                        </button>

                    </div>

                    <FilterField style={{ height: 28, opacity: hiddenCols['Unit'] ? 0.6 : 1 }}>
                ...
                <div style={{ marginBottom: 5 }}>
                    ...
                    <div style={{ fontSize: 9, color: 'var(--color-text-muted)', marginBottom: 4, letterSpacing: 0.3, display: 'flex', alignItems: 'center', gap: 4 }}>

                        <span style={{ textDecoration: hiddenCols['Macadam'] ? 'line-through' : 'none', opacity: hiddenCols['Macadam'] ? 0.5 : 1 }}>Macadam</span>

                        <button type="button" onClick={() => setHiddenCols(prev => ({ ...prev, 'Macadam': !prev['Macadam'] }))}

                            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', padding: 0 }}>

                            {hiddenCols['Macadam'] ? <EyeOff size={10} /> : <Eye size={10} />}

                        </button>

                    </div>

                    <div style={{ opacity: hiddenCols['Macadam'] ? 0.6 : 1 }}>
                ...
                <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        ...
                        <span style={{ fontSize: 8, color: 'var(--color-text-muted)', textDecoration: hiddenCols['Space Match (%)'] ? 'line-through' : 'none', opacity: hiddenCols['Space Match (%)'] ? 0.5 : 1 }}>Space Match (%)</span>

                        <button type="button" onClick={() => setHiddenCols(prev => ({ ...prev, 'Space Match (%)': !prev['Space Match (%)'] }))}

                            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', padding: 0 }}>

                            {hiddenCols['Space Match (%)'] ? <EyeOff size={9} /> : <Eye size={9} />}

                        </button>

                    </div>

                    <div style={{ opacity: hiddenCols['Space Match (%)'] ? 0.5 : 1, textDecoration: hiddenCols['Space Match (%)'] ? 'line-through' : 'none' }}>
...
function fmtAttr(v) {
    ...
    return v.replace(/DEG/g, '°').replace(/_/g, ' ').replace('TUNABLE', '🌡️ Tunable');
...
const Step4Recommendations = memo(function Step4Recommendations({ items, setItems, activeLabels, toggleLabel, gstRate, products, quotationId, customLabels, onRenameLabel, customCols, setCustomCols, hiddenCols, setHiddenCols }) {
    ...
    return (
        ...
        <div>
                ...
                <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 900 }}>
                    ...
                    <thead>
                        ...
                        <tr style={{ background: 'var(--color-base)', borderBottom: '1px solid var(--color-border)' }}>
                            ...
                            <th style={{ padding: '10px 12px', fontSize: 10, color: 'var(--color-text-muted)', textAlign: 'left', whiteSpace: 'nowrap', fontWeight: 600, letterSpacing: 0.5, minWidth: 280 }}>

                                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>

                                    <span style={{ textDecoration: hiddenCols['Description / Attributes'] ? 'line-through' : 'none', opacity: hiddenCols['Description / Attributes'] ? 0.5 : 1 }}>PRODUCT</span>

                                    <button type="button" onClick={() => setHiddenCols(prev => ({ ...prev, 'Description / Attributes': !prev['Description / Attributes'] }))}
                                        ...
                                        {hiddenCols['Description / Attributes'] ? <EyeOff size={12} /> : <Eye size={12} />}
                            ...
                            <th style={{ padding: '10px 6px', fontSize: 10, color: 'var(--color-text-muted)', textAlign: 'center', minWidth: 80 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                                    ...
                                    <button type="button" onClick={() => setHiddenCols(prev => ({ ...prev, 'Unit': !prev['Unit'] }))}
                                        ...
                                        {hiddenCols['Unit'] ? <EyeOff size={12} /> : <Eye size={12} />}
                            ...
                            {activeLabels.map(label => {
                                ...
                                const headerLabel = getCustomLabel(customLabels, `rec_${label}_header`, `REC ${label}`);
                                ...
                                return (
                                    ...
                                    <th key={label} style={{ padding: '10px 4px', fontSize: 10, color: REC_COLORS[label], textAlign: 'center', minWidth: 130, fontWeight: 800 }}>
                                        ...
                                        <div style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                                            ...
                                            <span style={{ textDecoration: hiddenCols[headerLabel] ? 'line-through' : 'none', opacity: hiddenCols[headerLabel] ? 0.5 : 1 }}>{headerLabel}</span>

                                            <button type="button" onClick={() => setHiddenCols(prev => ({ ...prev, [headerLabel]: !prev[headerLabel] }))}

                                                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', display: 'inline-flex', alignItems: 'center', padding: 0 }}>

                                                {hiddenCols[headerLabel] ? <EyeOff size={12} /> : <Eye size={12} />}
                                    ...
                                    <motion.tr key={item._tempId || item.id || idx}
                                        ...
                                        {activeLabels.map(label => (
                                            ...
                                            <RecCell key={label} label={label}
                                                ...
                                                customLabels={customLabels} onRenameLabel={onRenameLabel} hiddenCols={hiddenCols} setHiddenCols={setHiddenCols} />
            ...
            {/* Ã¢€—Ã¢€— Attribute Editor Modal Ã¢€—Ã¢€— */}
            ...
            <div style={{ marginBottom: 16, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 6, alignItems: 'center' }}>
                    ...
                    {[
                        ...
                        { key: 'Product Image', label: '🖼️ Photo', tip: 'in PDF' },
